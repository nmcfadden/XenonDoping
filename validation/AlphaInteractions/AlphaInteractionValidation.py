...
def runSimulation(t):
    ...
    dictionary = {}
    token = ['AI_NumberOfSimulatedInteractions','AI_RunSimulation','AI_RelativeComparison','RC_Version', 'RC_newVersion']
    ...
    if dictionary[token[1]][0] == '1':
        ...
        os.system('macros/alphaSimulation.py ' + element)
        if os.path.exists('dat/' + element + '.dat') == True:
            print '+++ analysing \'' + element + ' Decay\' data +++'
            os.system('./AlphaInteractionAnalysis ' + element + ' ' + dictionary[token[3]][:-1] + ' ' + dictionary[token[4]][:-1] + ' > /dev/null')

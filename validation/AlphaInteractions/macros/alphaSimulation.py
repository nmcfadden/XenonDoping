...
def main():
    energy = readFile('dat/alphaEnergies.txt')
    ...
    for i in range(len(energy)):
        ...
        for j in range(len(macro)):
            ...
            if substring[0] == '/MG/geometry/GDML/sourceFile':
                ...
                macro[j] = substring[0] + ' ' + magePath + '/validation/AlphaInteractions/macros/' + element + '.gdml\n'
            if substring[0] == '/MG/eventaction/rootfilename':
                magePath = str(os.path.expandvars('$MAGEDIR'))
                macro[j] = substring[0] + ' ' + magePath + '/validation/AlphaInteractions/dat/' + element + '_' + energy[i][:-1] + 'MeV.root\n'

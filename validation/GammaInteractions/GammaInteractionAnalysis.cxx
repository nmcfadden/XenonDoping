...
int main(int argc, char* argv[]) {
	...
	if (argc < 2) {
		std::cout << "Usage: GammaInteractionAnalysis <element name> \n";
		return 0;
	}
	...
	vector<double> attenuation_simulation_sigma;
	vector<string> token;
	double density,temp;
	int dataType;
	size_t endposition;
	...
	inputfile.open("dat/gammaEnergies.txt");
	if (inputfile.fail()) {
		cout << "ERROR! File gammaEnergies.txt is missing. GammaInteractionAnalysis terminated\n";
		return 0;
	}
	...
	while (getline(inputfile, s)) {
		...
		newInteraction->SetToken("XX:" + elementName + "_attenuation_gamma" + s + "MeV:XX");
		...
		outputfile << newInteraction->GetAttenuationSimulationToken() << "\n";
		...
		if(newInteraction->GetEnergy() == 2.0){
			...
			label.str("");
			label.clear();
			label << "pics/" << elementName << "_gamma" << newInteraction->GetEnergy() << "MeV.pdf";
			can->SaveAs(label.str().c_str());
			outputfile << "XX:" << elementName << "_gamma" << newInteraction->GetEnergy() << "MeV_plot:XX\t";
			outputfile << "GammaInteractions/" << label.str() << "\n";
			gSystem->ProcessEvents();
		}

		attenuation_simulation.push_back(newInteraction->GetAttenuationSimulation());
		attenuation_simulation_sigma.push_back(newInteraction->GetAttenuationSimulationSigma());
		//************************************************************
	...
	for(int i=0;i<attenuation_simulation.size();i++){
		...
		gr_sim->SetPointError(i,0.0,attenuation_simulation_sigma[i]);


		if(relcomp){
			s = token[i];
			outputfile << s.substr(0,s.size()-3) << "_attenuation_simulation_old:XX\t";
			outputfile << rcomp->GetEntryValue(s.substr(0,s.size()-3) + "_attenuation_simulation:XX") << " $\\pm$ " << rcomp->GetEntrySigma(s.substr(0,s.size()-3) + "_attenuation_simulation:XX") << "\n";
			outputfile << s.substr(0,s.size()-3) << "_attenuation_reldiff:XX\t";
			outputfile << rcomp->GetAgreementColourString(attenuation_simulation[i],attenuation_simulation_sigma[i],rcomp->GetEntryValue(s.substr(0,s.size()-3) + "_attenuation_simulation:XX"),rcomp->GetEntrySigma(s.substr(0,s.size()-3) + "_attenuation_simulation:XX"),10.0 );
			rcomp_new->WriteEntry(s.substr(0,s.size()-3) + "_attenuation_simulation:XX",attenuation_simulation[i],attenuation_simulation_sigma[i]);
	...
	gr_sim->GetXaxis()->SetTitle("photon energy [MeV]");
	gr_sim->GetYaxis()->SetTitle("attenuation coefficient [1/cm]");
	...
	outputfile << "GammaInteractions/pics/" << elementName << "_plot.pdf\n";

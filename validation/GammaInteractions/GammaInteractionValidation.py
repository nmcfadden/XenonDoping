...
def runSimulation(t):
    dictionary = {}
    token = ['GI_NumberOfSimulatedInteractions','GI_RunSimulation','GI_RelativeComparison','RC_Version', 'RC_newVersion']
...
# creating dictionary of MaGe validation config file  
    ...
    if dictionary[token[1]][0] == '1':
        ...
        os.system('macros/gammaSimulation.py ' + element)
        if os.path.exists('dat/' + element + '.dat') == True:
            print '+++ analysing \'' + element + ' Decay\' data +++'
	    print './GammaInteractionAnalysis ' + element + ' ' + dictionary[token[3]][:-1] + ' ' + dictionary[token[4]][:-1] + ' > /dev/null'
            os.system('./GammaInteractionAnalysis ' + element + ' ' + dictionary[token[3]][:-1] + ' ' + dictionary[token[4]][:-1] + ' > /dev/null')

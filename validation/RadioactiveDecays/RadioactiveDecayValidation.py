...
# creating dictionary of MaGe validation config file  
    ...
    if dictionary[token[1]][0] == '1':
        ...
        os.system('DecayEngine ' + t + ' dat/' + element + '.root' + '>/dev/null')
        if os.path.exists('dat/' + element + '.dat') == True:
            print '+++ analysing \"' + element + ' Decay\" data +++'
            os.system('./DecayAnalysis dat/' + element + '.dat dat/' + element + '.root ' + dictionary[token[3]][:-1] + ' ' +  dictionary[token[4]][:-1] + ' > /dev/null')
            ...
            os.system("rm -f dat/" + element + ".root")
        else:

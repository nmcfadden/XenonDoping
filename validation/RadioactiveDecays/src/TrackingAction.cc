...
void TrackingAction::PreUserTrackingAction(const G4Track* track)
{
  ...
  //
  G4int ih = 0;
  if (particle == G4Electron::Electron())  ih = 1;
  else if (particle == G4Positron::Positron()) ih = -1;
  else if (particle == G4NeutrinoE::NeutrinoE()) ih = 2;
  else if (particle == G4AntiNeutrinoE::AntiNeutrinoE()) ih = -2;
  ...
  TRootWriter::GetInstance()->AddEvent(EID,ih,track->GetGlobalTime(),Ekin);
  ...
  if (charge > 2.) {
    ...
    //G4cout << EID << "\n";
    G4String s = name.substr(0,4); // this line used to segfault when it was: name.substr(name.length()-5);
    if(s == "[0.0]" && ID > 1){
    	//G4cout << name << "\t" << "deexcited!\n";

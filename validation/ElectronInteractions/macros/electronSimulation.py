...
def main():
    energy = readFile('dat/electronEnergies.txt')
    ...
    for i in range(len(energy)):
        ...
        for j in range(len(macro)):
            ...
            if substring[0] == '/MG/geometry/GDML/sourceFile':
                ...
                macro[j] = substring[0] + ' ' + magePath + '/validation/ElectronInteractions/macros/' + element + '.gdml\n'
            if substring[0] == '/MG/eventaction/rootfilename':
                magePath = str(os.path.expandvars('$MAGEDIR'))
                macro[j] = substring[0] + ' ' + magePath + '/validation/ElectronInteractions/dat/' + element + '_' + energy[i][:-1] + 'MeV.root\n'
...
#        os.system('MaGe ' + file + ' > debug/' + element + '_' + energy[i][:-1] + 'MeV.txt')

...
void MGOutputMCOpticalRun::RootSteppingAction(const G4Step* step)
{
  ...
  */
  ...
  G4String physVolName = postStepPoint->GetPhysicalVolume()->GetName();
  G4String prePhysVolName = preStepPoint->GetPhysicalVolume()->GetName();
  ...
  if(creatorName != "Scintillation" && iStep > 0) {
    if ( !fWriteAllSteps){
      if(fWriteAllStepsInEventsThatDepositEnergy && eDep == 0) return;
      else if(sensVolID == 0 && fWriteAllSensitiveSteps){
        return;

 ...
 * 
 * 05-22-2013 modified to include the possibility of using a collimator, e.g. for xy scan, instead of a source holder,  W. Xu
 *
 * 06-14-2013 modified the setscrew positions to be flexible, in-suit changing with the outer radius of crystal. The diameter of Teflon Ring should have 2*0.4mm, instead of 0.4mm. W. Xu
 */
//---------------------------------------------------------------------------//
//
...
MJGeometryPopTopXYscan::MJGeometryPopTopXYscan(G4String version): 
MGGeometryDetector(version)
{
  ConstructMaterials();
  fCrystal= new MJGeometryOrtecCrystal();
  fCrystalOffsetFromInsideOfVacuumShroud=5.0*mm;//Quantity G in the ORTEC spec sheets
  ...
  fPlaceSource=true;
  fPlaceCollimator=true;
  fPlaceSourceHolder=false; //by defualt, either collimator or the sourceholder

  fSourceX=0.*mm;
  fSourceY=0.*mm;
  if(fPlaceCollimator)fSourceZ=(3.175*25.4+0.001)*mm+fCollimatorGap; //the collimator is 3.175"
  else fSourceZ=25.0*cm;
  //Check All dimensions for the Collimator
  ...
  fG4Messenger=new MJGeometryPopTopXYscanMessenger(this);
  
}

MJGeometryPopTopXYscan::~MJGeometryPopTopXYscan()
{
  
  
}

void MJGeometryPopTopXYscan::ConstructMaterials()
{
  G4String symbol;  
  ...
  /*G4Material* Lead =*/ new G4Material("Lead", z=82.,  a=207.2*g/mole, density=11.34*g/cm3);
...
void MJGeometryPopTopXYscan::ConstructDetector()
{
  ...
  G4double SetScrewRadialPosition=fIRCupOuterRadius-0.5*fIRCupThickness; //R = center of the IR shield wall
  ...
  *SetScrew2Affine1 *= *SetScrew2Affine2;  
  ...
  //The crystal
  G4double ZCrystalBottom=-(fVacuumShroudEndcapThickness+fCrystal->GetCrystalLength()+fCrystalOffsetFromInsideOfVacuumShroud); //The top of the cryostat is defined to be Z=0
  ...
  //Place the setscrews
  name="SetScrew";
  G4Tubs* SetScrewSolid= new G4Tubs("SetScrewSolid",0.,0.5*fSetScrewDiameter-IRCupTolerance,0.5*fSetScrewLength-IRCupTolerance,0.,2.*pi);
  ...
  G4ThreeVector  *SetScrew1RealPosition = new G4ThreeVector((0.5*fCrystal->GetCrystalDiameter()+0.5*fSetScrewLength+IRCupTolerance)*cos(SetScrew1Angle),
                                                            (0.5*fCrystal->GetCrystalDiameter()+0.5*fSetScrewLength+IRCupTolerance)*sin(SetScrew1Angle),
                                                            IRCupZTop-fSetScrewHeightFromTopOfCup);
    
  G4ThreeVector  *SetScrew2RealPosition = new G4ThreeVector((0.5*fCrystal->GetCrystalDiameter()+0.5*fSetScrewLength+IRCupTolerance)*cos(SetScrew2Angle),
                                                            (0.5*fCrystal->GetCrystalDiameter()+0.5*fSetScrewLength+IRCupTolerance)*sin(SetScrew2Angle),
                                                            IRCupZTop-fSetScrewHeightFromTopOfCup);
    
  new G4PVPlacement(SetScrew1Rotation, *SetScrew1RealPosition, SetScrewLogical,name+"1_Physical",theDetectorLogical,false,0,false);
  new G4PVPlacement(SetScrew2Rotation, *SetScrew2RealPosition, SetScrewLogical,name+"2_Physical",theDetectorLogical,false,0,false);
  ...
  if(fPlaceSource){
    if(fPlaceCollimator){
	fSourceZ=(3.175*25.4+0.001)*mm+fCollimatorGap;
	MGLog(routine)<<"collimator used, the Z position of the source is re-set to be the top of the collimator"<<endl;
    }	
    G4ThreeVector* sourcePosition = new G4ThreeVector(fSourceX, fSourceY,fSourceZ);
    MGLog(routine)<<"source geometry: X="<<fSourceX<<" Y="<<fSourceY<<" Z="<<fSourceZ<<endl;
  ...
  if(fPlaceCollimator){
    name="Collimator";
    //G4double CollimatorTol=0.001*mm;

    MGLog(routine)<<"Collimator geometry: Z="<<(1.5875*25.4*mm+fCollimatorGap)<<endl; 
    
    G4Box* CollimatorSolid = new G4Box(name+"_Solid",0.9*25.4*mm,0.9*25.4*mm,1.5875*25.4*mm);//the length is 3.175" 
    G4Tubs* Collimatorhole = new G4Tubs(name+"_Hole",0,0.059*25.4*mm,1.5875*25.4*mm,0*deg,360*deg);
    G4SubtractionSolid* Collimator=new G4SubtractionSolid(name,CollimatorSolid,Collimatorhole,0, G4ThreeVector(0, 0, 0));

    G4LogicalVolume *CollimatorLogical = new G4LogicalVolume(Collimator,G4Material::GetMaterial("Lead"),name+"_Logical");    
    CollimatorLogical->SetVisAttributes(new G4VisAttributes(G4Colour(0.2,0.3,0.5))); 
    G4ThreeVector  *CollimatorPosition = new G4ThreeVector(fSourceX,fSourceY,(1.5875*25.4*mm+fCollimatorGap));
    G4RotationMatrix *CollimatorRotation = new G4RotationMatrix();	
    new G4PVPlacement(CollimatorRotation, *CollimatorPosition, CollimatorLogical,name+"_Physical",theDetectorLogical,false,0,false);
...
void MJGeometryPopTopXYscan::AddIRCupRing(G4double argDistanceFromTopOfCup, G4double argWidthOfRing,  G4double argThicknessOfRing, G4double argTolerance)
{
  G4double IRCupInnerR=fIRCupOuterRadius-fIRCupThickness;
  G4double IRCupZTop=-(fVacuumShroudEndcapThickness+fIRCupOffsetFromInsideOfVacuumShroud);
...
int MJGeometryPopTopXYscan::GetSensitiveVolumeID(const string& volName)
{

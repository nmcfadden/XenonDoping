...
LGND_JSONReader::LGND_JSONReader() :
  fDelimiters("\t :\",")
{
  fCurrentLine.clear();
...
LGND_JSONReader::LGND_JSONReader(const G4String &inputFile) :
  fDelimiters("\t :\",")
{
  fCurrentLine.clear();
  char* env = std::getenv("MAGEDIR");
...
std::vector<G4double> LGND_JSONReader::ReadDetectorRis(const G4String &serialNum)
{
  std::vector<G4double> badExit;
  //MGLog(trace) << "Reading the detector inner R's" << endlog;
  if(fInputFile.is_open())
    {
      ...
      G4String line;
      while (!fInputFile.eof()&&(!isDetectorType)){
...
std::vector<G4double> LGND_JSONReader::ReadDetectorRos(const G4String &serialNum)
{
  std::vector<G4double> badExit;
  //MGLog(trace) << "Reading the detector outer R's" << endlog;
  if(fInputFile.is_open()){
    fInputFile.seekg(0);
    G4bool foundSerialNum = false;
    G4bool isDetectorType = false;
...
//-----------------------
std::vector<G4double> LGND_JSONReader::ReadDetectorZs(const G4String &serialNum)
{
  std::vector<G4double> badExit;
  if(fInputFile.is_open())
    {
...
//-----------
std::vector<G4String> LGND_JSONReader::ReadCryostats(const G4String &serialNum)
{
  std::vector<G4String> badExit;
  MGLog(trace) << "Reading the cryostats" << endlog;
  if(fInputFile.is_open())
    {
      fInputFile.seekg(0);
...
//-----------
std::vector<G4String> LGND_JSONReader::ReadCrystalAssemblyList(const G4String &serialNum)
{
  std::vector<G4String> badExit;
  if(fInputFile.is_open())
    {
...
//----------------
std::vector<G4String>LGND_JSONReader::ReadAssemblyStringList (const G4String &serialNum)
{
  std::vector<G4String> badExit;
  if(fInputFile.is_open())
    {
...
//-------
std::vector<G4String> LGND_JSONReader::ReadCrystalAssembly(const G4String &serialNum)
{
  std::vector<G4String> badExit;
  if(fInputFile.is_open())
    {
      ...
      G4String line;
      while (!fInputFile.eof()&&(!isCrysAssem)){
        foundSerialNum = SearchFromCurrentPos(serialNum);
        getline(fInputFile, line);
        fCurrentLine = Tokenize(fDelimiters, line);
        if(fCurrentLine.size()>1)
...
//-----------
std::vector<G4String> LGND_JSONReader::ReadCustom(const G4String &serialNum, const G4String &type, const G4String &param)
{
   std::vector<G4String> badExit;
  if(fInputFile.is_open())
    {
...
G4bool LGND_JSONReader::SearchFromCurrentPos(G4String parameter) //WARNING: This changes location in the ifstream
{
...
//----------
std::vector<G4String> LGND_JSONReader::Tokenize(G4String separators, G4String input)
{

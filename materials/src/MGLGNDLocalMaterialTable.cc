...
//
// CLASS IMPLEMENTATION:  MGLGNDLocalMaterialTable.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
MGLGNDLocalMaterialTable::MGLGNDLocalMaterialTable():
...
void MGLGNDLocalMaterialTable::BuildIsotopes()
{
  if(!fIsotopesBuilt) {
    MGLog(routine) << "Building LGND-related isotopes." << endlog;
    ...
    MGLog(routine) << "Built LGND-related isotopes." << endlog;
    fIsotopesBuilt = true;
  } else
...
void MGLGNDLocalMaterialTable::BuildElements()
{
  // Note:
  ...
  if(!fElementsBuilt) {
    ...
    MGLog(routine) << "Building LGND-related Elements." << endlog;
    ...
    LGNDElement = new G4Element("Germanium-Enr86", "Ge", 2);
    LGNDElement->AddIsotope(G4Isotope::GetIsotope("Germanium74"), 0.14);
    LGNDElement->AddIsotope(G4Isotope::GetIsotope("Germanium76"), 0.86);
    ...
    MGLog(routine) << "LGND-related materials built." << endlog;
    fElementsBuilt = true;
  } else
...
void MGLGNDLocalMaterialTable::BuildMaterials()
{
  if(!fMaterialsBuilt) {
    MGLog(routine) << "Building LGND-related materials." << endlog;
...
void MGLGNDLocalMaterialTable::BuildAll()
{
  BuildIsotopes();
  BuildElements();
  BuildMaterials();
  //Register optical Materials
  MGLGNDOpticalMaterialProperties * opticalMaterials = new MGLGNDOpticalMaterialProperties();

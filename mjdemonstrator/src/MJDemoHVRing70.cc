...
//      
// CLASS IMPLEMENTATION:  MJDemoHVRing70.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
MJDemoHVRing70::MJDemoHVRing70(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "MJ80-02-007", "Copper-EF")
{;}

MJDemoHVRing70::MJDemoHVRing70(const MJDemoHVRing70 & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoHVRing70::~MJDemoHVRing70()
{;}

G4LogicalVolume* MJDemoHVRing70::ConstructPart()
{
  ...
  if (pVol == NULL){
    ...
    G4double rInner[] = {1.3975*25.4*mm, 1.3975*25.4*mm, 1.25*25.4*mm, 1.25*25.4*mm};
    G4double rOuter[] = {1.4575*25.4*mm, 1.4575*25.4*mm, 1.4575*25.4*mm, 1.4575*25.4*mm};
    ...
    G4UnionSolid* HVRing1flange = new G4UnionSolid("HVRing1flange", body, flangeFilletHole,
                                                   0, G4ThreeVector(1.7*25.4*mm, 0, -0.03*25.4*mm));
    ...
    G4UnionSolid* HVRing2flanges = new G4UnionSolid("HVRing2flanges", HVRing1flange, 
                                        ...
                                        G4ThreeVector(-0.85*25.4*mm, 1.472*25.4*mm, -0.03*25.4*mm));

    G4RotationMatrix* rot240deg = new G4RotationMatrix();
    rot240deg->rotateZ(-240*deg);                            
    G4UnionSolid* HVRing = new G4UnionSolid("HV Ring", HVRing2flanges, 
                                                   flangeFilletHole, rot240deg, 
                                        G4ThreeVector(-0.85*25.4*mm, -1.472*25.4*mm, -0.03*25.4*mm));                                
    ...
    MGLog(debugging) << "Created HV Ring Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing HV Ring Logical" << endlog; 
  return pVol;
}  

...
//      
// CLASS IMPLEMENTATION:  MJDemoThermosyphonSupportThinHalfMale.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
#include "mjdemonstrator/MJDemoThermosyphonSupportThinHalfMale.hh"
...
MJDemoThermosyphonSupportThinHalfMale::MJDemoThermosyphonSupportThinHalfMale(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "MJ80-11-137-R002", "Vespel")
{;}

MJDemoThermosyphonSupportThinHalfMale::MJDemoThermosyphonSupportThinHalfMale(const MJDemoThermosyphonSupportThinHalfMale & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoThermosyphonSupportThinHalfMale::~MJDemoThermosyphonSupportThinHalfMale()
{;}

G4LogicalVolume* MJDemoThermosyphonSupportThinHalfMale::ConstructPart()
{
  ...
  if (pVol == NULL){
    ...
    G4Tubs* mainCyl = new G4Tubs("Main Cyl",33.274/2*mm, 70.18/2*mm, 3.81/2*mm, M_PI, M_PI);
    G4Tubs* hole1 = new G4Tubs("Hole 1", 0, 12.954/2*mm, 10/2*mm, 0, 2*M_PI);
    G4Tubs* hole2 = new G4Tubs("Hole 2", 55.982/2*mm, 63.906/2*mm, 10/2*mm, 200*deg, 40*deg);
    G4Tubs* hole3 = new G4Tubs("Hole 3", 55.976/2*mm, 63.912/2*mm, 10/2*mm, 285*deg, 32*deg);
    G4Tubs* hole4 = new G4Tubs("Hole 4", 37.44/2*mm, 45.364/2*mm, 10/2*mm, 282*deg, 32*deg);
    G4Tubs* hole5 = new G4Tubs("Hole 5", 37.44/2*mm, 45.364/2*mm, 10/2*mm, 191*deg, 46*deg);
    G4Tubs* holeEnd = new G4Tubs("Cylindrical Hole Ends", 0, 3.692/2*mm, 10/2*mm, 0, 2*M_PI);
    G4Tubs* holeBand1 = new G4Tubs("Hole Band 1", 35.306/2*mm, 38.1/2*mm, 10/2*mm, -30.9*deg, 60*deg);
    G4Tubs* holeBand2 = new G4Tubs("Hole Band 2", 35.306/2*mm, 38.1/2*mm, 10/2*mm, -23.9*deg, 60*deg);
    G4Tubs* screwHole = new G4Tubs("Screw Hole", 0, 3.65/2*mm, 10/2*mm, 0, 2*M_PI);
    G4Box* piecePrism1 = new G4Box("Piece Prism 1", 9.113/2*mm, 6.05/2*mm, 3.81/2*mm);
    G4Box* piecePrism2 = new G4Box("Piece Prism 2", 3.175/4*mm, (6.05-3.175/2)/2*mm+epsilon, 3.81/2*mm+epsilon);
    G4Tubs* pieceCyl = new G4Tubs("Piece Cyl", 0, 3.175/2*mm, 6.35/2*mm+epsilon, 0, 2*M_PI);
    G4Box* tab = new G4Box("tab", 11.303/2*mm, 3.175/2*mm, (2.794+1.905)/2*mm);
    G4Tubs* tubeclearance = new G4Tubs("tubeclearance", 0, 34.1/2*mm, 5*mm, 0, 2*M_PI);
    ...
    partUnion = new G4UnionSolid("Part", partUnion, tab, rotN, G4ThreeVector((16.637+11.303/2)*mm, -3.175/2*mm, (2.794+1.905)/2*mm));
      
    partSub = new G4SubtractionSolid("Part", partUnion, tubeclearance, rotN, G4ThreeVector(0,0,(3.81/2+4.99)*mm));
    ...
    MGLog(debugging) << "Created Thermosyphon Support Thin Half With Slotted Cable Holes Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing Thermosyphon Support Thin Half With Slotted Cable Holes Logical" << endlog; 
  return pVol;
}  

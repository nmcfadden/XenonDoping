...
//      
// CLASS IMPLEMENTATION:  MJChar832Screw.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
 * 
 *Class for a generic 8-32 hex head bolt.  Part origin is 
 *coaxial with screw, coincident with shoulder.
 *
 * Modified from MJDemo832Screw.cc to change material to stainless steel
 */
// 
// --------------------------------------------------------------------------//
/**
...
MJChar832Screw::MJChar832Screw(G4String partName, G4String serialNumber, 
								G4double length) :
  MJVDemoPart(partName, serialNumber, "8-32", "Copper-EF"), 
  fScrewLength(length*mm)
{;}

MJChar832Screw::MJChar832Screw(const MJChar832Screw & rhs) : 
  MJVDemoPart(rhs), fScrewLength(rhs.fScrewLength)
{;}

MJChar832Screw::~MJChar832Screw()
{;}

G4LogicalVolume* MJChar832Screw::ConstructPart()
{
  ...
  if (pVol == NULL){
    ...
    // Definition of stainless steel, replacing Copper-EF
    // Compositions taken from azom.com, Grade 316 SS (UNS S31600)
    ...
    G4Material* stainlessSteel = new G4Material(name="StainlessSteel316Grade",density=8000*kg/m3,ncomponents=10);
    ...
    // Definition of screw, made from hexagonal prism attached to a cylinder
    // Dimensions based off McMaster-Carr 32196A205 Socket Head Cap Screw
    G4double zPlanes[] = {0*25.4*mm, 0.164*25.4*mm};
    G4double rInner[] = {0, 0};
    G4double rOuter[] = {0.135*25.4*mm, 0.135*25.4*mm};
    G4Polyhedra* hex = new G4Polyhedra("hex", pi/2, 2*pi, 6, 2, zPlanes, rInner, rOuter);    

    G4Tubs* thread = new G4Tubs("thread", 0, 0.082*25.4*mm, fScrewLength/2 + 0.1*mm, 0, 2*pi);//0.0724
    ...
    // material lookup is overridden to place custom material

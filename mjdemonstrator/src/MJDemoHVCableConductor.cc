...
//      
// CLASS IMPLEMENTATION:  MJDemoHVCableConductor.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
MJDemoHVCableConductor::MJDemoHVCableConductor(G4String partName, G4String serialNumber, 
								G4double length) :
  MJVDemoPart(partName, serialNumber, "HVwire", "Copper-EF"), 
  fWireLength(length)
{;}

MJDemoHVCableConductor::MJDemoHVCableConductor(const MJDemoHVCableConductor & rhs) : 
  MJVDemoPart(rhs), fWireLength(rhs.fWireLength)
{;}

MJDemoHVCableConductor::~MJDemoHVCableConductor()
{;}

G4LogicalVolume* MJDemoHVCableConductor::ConstructPart()
{
  ...
  if (pVol == NULL){
    ...
    G4Tubs* wire = new G4Tubs("wire", 0, 0.01*25.4*mm, fWireLength/2, 0, 2*pi);
    ...
    MGLog(debugging) << "Created " << fWireLength << "mm HV Wire Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing " << fWireLength << "mm HV Wire Logical" << endlog;
  return pVol;
}  

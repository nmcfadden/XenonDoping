...
MJDemoPoly_Panel5::MJDemoPoly_Panel5(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "PolyNorthWall_Big", serialNumber)
{;}

MJDemoPoly_Panel5::MJDemoPoly_Panel5(const MJDemoPoly_Panel5 & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoPoly_Panel5::~MJDemoPoly_Panel5()
{;}

G4LogicalVolume* MJDemoPoly_Panel5::ConstructPart()
{
  ...
  if (pVol == NULL){
		G4Box* poly_Panel5 = new G4Box("poly_Panel5", 1085.85, 25.4*mm, 927.1*mm);  //rawPanel  73 x 85.5
	

    G4VisAttributes* greenVisAtt = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0)); // green
    greenVisAtt->SetForceWireframe( false );
    G4Material *material = G4Material::GetMaterial(this->GetMaterial());
    pVol = new G4LogicalVolume(poly_Panel5, material, logicalName);

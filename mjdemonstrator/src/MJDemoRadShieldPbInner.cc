...
//      
// CLASS IMPLEMENTATION:  MJDemoRadShieldPbInner.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
MJDemoRadShieldPbInner::MJDemoRadShieldPbInner(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "ShieldPbInnerDwg", "Lead-Ain")
{;}

MJDemoRadShieldPbInner::MJDemoRadShieldPbInner(const MJDemoRadShieldPbInner & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoRadShieldPbInner::~MJDemoRadShieldPbInner()
{;}

G4LogicalVolume* MJDemoRadShieldPbInner::ConstructPart()
{
  ...
  if (pVol == NULL){
    G4Box* body = new G4Box("body", 20*25.4*mm+5.0*cm, 13*25.4*mm+5.0*cm, 14*25.4*mm+5.0*cm);
...
*/
    ...
    G4SubtractionSolid* leadShieldInner = new G4SubtractionSolid("leadShieldInner", body7, tubeCut, rotation2,
    ...
    pVol = new G4LogicalVolume(leadShieldInner, material, logicalName);
    pVol->SetVisAttributes(blackVisAtt); 
    MGLog(debugging) << "Created Lead Shield Inner Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing Lead Shield Inner Logical" << endlog; 
  return pVol;
}  

...
//      
// CLASS IMPLEMENTATION:  MJDemoRadShieldN2VolumeEast.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
 * Part origin:  Box Center  (8.015" East of shield center)
 *
 */
// 
//---------------------------------------------------------------------------//
...
MJDemoRadShieldN2VolumeEast::MJDemoRadShieldN2VolumeEast(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "ShieldN2EastDwg", "Nitrogen-Gas")
{;}

MJDemoRadShieldN2VolumeEast::MJDemoRadShieldN2VolumeEast(const MJDemoRadShieldN2VolumeEast & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoRadShieldN2VolumeEast::~MJDemoRadShieldN2VolumeEast()
{;}

G4LogicalVolume* MJDemoRadShieldN2VolumeEast::ConstructPart()
{
  ...
  if (pVol == NULL){
      ...
      G4SubtractionSolid* body2 = new G4SubtractionSolid("body2", body, cryostatTube, rotation1,
                                                         G4ThreeVector(7.0*25.4*mm, 0, 4.4265*25.4*mm));
    ...
    MGLog(debugging) << "Created East N2 Volume Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing East N2 Volume Logical" << endlog;
  return pVol;
}  

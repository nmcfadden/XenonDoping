...
MJDemoCryostatHoop::MJDemoCryostatHoop(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "CryostatHoopDwg", "Copper-EF")
{;}

MJDemoCryostatHoop::MJDemoCryostatHoop(const MJDemoCryostatHoop & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoCryostatHoop::~MJDemoCryostatHoop()
{;}

G4LogicalVolume* MJDemoCryostatHoop::ConstructPart()
{
  ...
  if (pVol == NULL){
    ...
    G4double zConePlanes2[] = {-1*25.4*mm, 0, 0, 1*25.4*mm};
    G4double rConeInner2[] = {0, 0, 0, 0};
    G4double rConeOuter2[] = {1.75*25.4*mm, 1.75*25.4*mm, 1.9125*25.4*mm, 1.9125*25.4*mm};
    G4Polycone* tubeCut = new G4Polycone("TubeCut", 0, 2*pi, 4, zConePlanes2, rConeInner2, rConeOuter2);

    G4RotationMatrix* rotation = new G4RotationMatrix();
    rotation->rotateY(-pi/2);
    G4SubtractionSolid* body = new G4SubtractionSolid("body", barrel, tubeCut, rotation,
    											G4ThreeVector(6.625*25.4*mm, 0, -3.0*25.4*mm));

    G4Tubs* tabSlot = new G4Tubs("TabSlot", 0, 0.80*25.4*mm, 0.250*25.4*mm, 2, 2*pi);//0.75, 0.065
    G4SubtractionSolid* body2 = new G4SubtractionSolid("body2", body, tabSlot, 0,
    		G4ThreeVector(-5.945*25.4*mm, 0, -4.7125*25.4*mm));
    G4SubtractionSolid* body3 = new G4SubtractionSolid("body3", body2, tabSlot, 0,
    		G4ThreeVector(-5.945*25.4*mm*cos(120*deg), -5.945*25.4*mm*sin(120*deg), -4.7125*25.4*mm));    											
    G4SubtractionSolid* hoop = new G4SubtractionSolid("hoop", body3, tabSlot, 0,
    		G4ThreeVector(-5.945*25.4*mm*cos(240*deg), -5.945*25.4*mm*sin(240*deg), -4.7125*25.4*mm));

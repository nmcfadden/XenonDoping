...
MJDemoPoly_Panel8::MJDemoPoly_Panel8(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "PolyNorthWall_SmallOuterLower", serialNumber)
{;}

MJDemoPoly_Panel8::MJDemoPoly_Panel8(const MJDemoPoly_Panel8 & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoPoly_Panel8::~MJDemoPoly_Panel8()
{;}

G4LogicalVolume* MJDemoPoly_Panel8::ConstructPart()
{
  ...
  if (pVol == NULL){
		G4Box* poly_Panel8 = new G4Box("poly_Panel8",  819.15*mm, 25.4*mm, 762*mm);  //64.5 x 60
	

    G4VisAttributes* greenVisAtt = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0)); // green
    greenVisAtt->SetForceWireframe( false );
    G4Material *material = G4Material::GetMaterial(this->GetMaterial());
    pVol = new G4LogicalVolume(poly_Panel8, material, logicalName);

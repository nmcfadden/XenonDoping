...
//      
// CLASS IMPLEMENTATION:  MJDemoRadShieldPbOuter.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
MJDemoRadShieldPbOuter::MJDemoRadShieldPbOuter(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "ShieldPbOuterDwg", "Lead-Ain")
{;}

MJDemoRadShieldPbOuter::MJDemoRadShieldPbOuter(const MJDemoRadShieldPbOuter & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoRadShieldPbOuter::~MJDemoRadShieldPbOuter()
{;}

G4LogicalVolume* MJDemoRadShieldPbOuter::ConstructPart()
{
  ...
  if (pVol == NULL){
    ...
    G4Box* pocket = new G4Box("pocket", 20.001*25.4*mm+5.0*cm, 13.001*25.4*mm+5.0*cm, 14.001*25.4*mm+5.0*cm);
...
*/
    ...
    G4SubtractionSolid* leadShieldOuter = new G4SubtractionSolid("leadShieldOuter", body7, tubeCut, rotation2,
    ...
    pVol = new G4LogicalVolume(leadShieldOuter, material, logicalName);
    pVol->SetVisAttributes(blackVisAtt); 
    MGLog(debugging) << "Created Lead Shield Outer Logical" << endlog;
  }
  else  MGLog(debugging) << "Using pre-existing Lead Shield Outer Logical" << endlog; 
  return pVol;
}  

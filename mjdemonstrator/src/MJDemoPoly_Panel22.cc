...
MJDemoPoly_Panel22::MJDemoPoly_Panel22(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "PolyEastWall_ModuleInnerUpper", serialNumber)
{;}

MJDemoPoly_Panel22::MJDemoPoly_Panel22(const MJDemoPoly_Panel22 & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoPoly_Panel22::~MJDemoPoly_Panel22()
{;}

G4LogicalVolume* MJDemoPoly_Panel22::ConstructPart()
{
  ...
  if (pVol == NULL){
		G4Box* box1 = new G4Box("box1", 25.4*mm, 1206.5*mm, 571.5*mm);    		//raw panel 95 x (9x5)
		G4Box* cutoutNitrogen = new G4Box("cutoutNitrogen", 25.5*mm, 50.8*mm, 63.5*mm);    	//4 x5 in 3rd panel from above
  	G4SubtractionSolid* poly_Panel22 = new G4SubtractionSolid("poly_Panel22", box1, cutoutNitrogen, 0, G4ThreeVector(0, -773.557*mm, 254*mm));
	

    G4VisAttributes* greenVisAtt = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0)); // green
    greenVisAtt->SetForceWireframe( false );
    G4Material *material = G4Material::GetMaterial(this->GetMaterial());
    pVol = new G4LogicalVolume(poly_Panel22, material, logicalName);

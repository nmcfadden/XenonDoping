...
//      
// CLASS IMPLEMENTATION:  MJProto7StringAssembly.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
MJProto7StringAssembly::MJProto7StringAssembly(G4String fName, G4String serialNumber) :
  MJVDemoAssembly(fName, serialNumber, "7Strings")
{
  ...
  std::vector<G4String> stringList = reader->ReadAssemblyStringList(serialNumber);
  delete reader;
...
MJProto7StringAssembly::MJProto7StringAssembly(const MJProto7StringAssembly & rhs) : 
  MJVDemoAssembly(rhs)
{
  fColdPlatePtr = new MJDemoColdPlate(fName+"_ColdPlate_001", "867-5309");
...
MJProto7StringAssembly::~MJProto7StringAssembly()
{
...
void MJProto7StringAssembly::Place(G4ThreeVector* assemPosition, 
                G4RotationMatrix* assemRotation, G4LogicalVolume* motherLogical)
{
  ...
  *SilicaStack7affine*=*assemAffine;
  ...
  if((fNumStrings > 1) && fString2Ptr)
    {
      G4ThreeVector *StringAssembly2localPos = new G4ThreeVector(stringRadius*cos(330*deg), stringRadius*sin(330*deg), -0.001*25.4*mm); //0
  ...
  if((fNumStrings > 2) && fString3Ptr)
    {
      G4ThreeVector *StringAssembly3localPos = new G4ThreeVector(stringRadius*cos(270*deg), stringRadius*sin(270*deg), -0.001*25.4*mm); //0
  ...
  if((fNumStrings >3) && fString4Ptr)
    {
      G4ThreeVector *StringAssembly4localPos = new G4ThreeVector(stringRadius*cos(210*deg), stringRadius*sin(210*deg), -0.001*25.4*mm); //0
  ...
  if((fNumStrings > 4) && fString5Ptr)
    {
      G4ThreeVector *StringAssembly5localPos = new G4ThreeVector(stringRadius*cos(150*deg), stringRadius*sin(150*deg), -0.001*25.4*mm); //0
  ...
  if((fNumStrings > 5) && fString6Ptr)
    {
      G4ThreeVector *StringAssembly6localPos = new G4ThreeVector(stringRadius*cos(90*deg), stringRadius*sin(90*deg), -0.001*25.4*mm); //0
  ...
  if((fNumStrings > 6) && fString7Ptr)
    {
      G4ThreeVector *StringAssembly7localPos = new G4ThreeVector(stringRadius*cos(30*deg), stringRadius*sin(30*deg), -0.001*25.4*mm); //0

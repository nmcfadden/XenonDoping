...
MJDemoPoly_Panel7::MJDemoPoly_Panel7(G4String partName, G4String serialNumber) :
  MJVDemoPart(partName, serialNumber, "PolyNorthWall_SmallInnerUpper", serialNumber)
{;}

MJDemoPoly_Panel7::MJDemoPoly_Panel7(const MJDemoPoly_Panel7 & rhs) : 
  MJVDemoPart(rhs)
{;}

MJDemoPoly_Panel7::~MJDemoPoly_Panel7()
{;}

G4LogicalVolume* MJDemoPoly_Panel7::ConstructPart()
{
  ...
  if (pVol == NULL){
		G4Box* poly_Panel7 = new G4Box("poly_Panel7", 742.95*mm, 25.4*mm, 381*mm);  //58.5 x (5x6)
	

    G4VisAttributes* greenVisAtt = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0)); // green
    greenVisAtt->SetForceWireframe( false );
    G4Material *material = G4Material::GetMaterial(this->GetMaterial());
    pVol = new G4LogicalVolume(poly_Panel7, material, logicalName);
